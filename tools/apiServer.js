...
server.post("/riskManagementQuestions/", function(req, res, next) {
  const error = validateQuestionnare(req.body);
  if (error) {
...
function validateQuestionnare(question) {
  if (!question.title) return "Title is required.";
  if (!question.authorId) return "Author is required.";
  return "";
}
